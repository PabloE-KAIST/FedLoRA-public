#!/usr/bin/env bash
# Standalone queue: AdaSparse-LoRA v2 — 3 rw × 3 gamma × 2 UL × 1 DL = 18 experiments per task.
#
# Env vars:
...
QUEUE_LOG="${FEDLORA_ROOT}/exp_standalone/standalone_queue_v2.log"
        ...
        for ul in "${UL_WINDOWS[@]}"; do
            for dl in "${DL_WINDOWS[@]}"; do
                ...
                if CUDA_VISIBLE_DEVICES=$GPU python federatedscope/main.py --cfg "$CONFIG" \
                    ...
                    glue.adapter.adasparse_lorav2.rank_max 200 \
                    ...
                    glue.adapter.adasparse_lorav2.stage2.residual_enabled True; then
...
log "[GPU ${GPU}] v2 queue complete for ${TASK}: ${PASS} passed, ${FAIL} failed"
echo "$(date '+%Y-%m-%d %H:%M:%S') SUMMARY v2__${TASK}: ${PASS} passed, ${FAIL} failed" >> "$QUEUE_LOG"
