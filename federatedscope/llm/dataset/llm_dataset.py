...
class LLMDataset(Dataset):
    ...
    def __init__(self,
        ...
        if use_chat_template and getattr(tokenizer, 'chat_template', None):
            # Instruction-tuned models: format the prompt with the tokenizer's
            # native chat template and open (but leave empty) the assistant turn
            # via add_generation_prompt=True. The target ({output}{eos}) then
            # continues that turn, and the loss mask (source_len) still masks the
            # whole templated prompt exactly as with the Alpaca template.
            sources = []
