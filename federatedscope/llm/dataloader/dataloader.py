...
def load_llm_dataset(config=None, **kwargs):
    ...
    if dataset_name.endswith('.json'):
        ...
        dataset = LLMDataset(
            list_data_dict, tokenizer,
            use_chat_template=getattr(config.llm, 'use_chat_template', False))
        ...
        list_data_dict = load_jsonl(fp,
                                    ...
                                    output='response',
                                    category='category')
        # FS-LLM (Table 6) uses ALL 8 Dolly NLP categories -> keep summarization by
        # default (the 8th client of a meta split). Drop it only if explicitly asked.
        if getattr(config.data, 'dolly_drop_summarization', False):
            train_data_dict = [
                x for x in list_data_dict if x["category"] != "summarization"
            ]
        else:
            train_data_dict = list_data_dict

        dataset = LLMDataset(
            train_data_dict, tokenizer,
            use_chat_template=getattr(config.llm, 'use_chat_template', False))
        #dataset = LLMDataset(list_data_dict, tokenizer)

        # Check if this is for hybrid splitter - if so, also load secondary dataset
        if hasattr(config.data, 'splitter') and config.data.splitter == 'hybrid_lda':
            # Load secondary dataset (Style B - Childish)
            secondary_fp = os.path.join(config.data.root, 'databricks-dolly-15k_Style_Childish.jsonl')
            if os.path.exists(secondary_fp):
                secondary_data_dict = load_jsonl(secondary_fp,
                                              instruction='instruction',
                                              input='context',
                                              output='response',
                                              category='category')
                # Filter out summarization from secondary dataset too
                secondary_train_data_dict = [
                    x for x in secondary_data_dict if x["category"] != "summarization"
                ]
                # Store secondary dataset in the dataset object for the splitter to access
                dataset.secondary_dataset = secondary_train_data_dict
                print(f"Loaded secondary dataset with {len(secondary_train_data_dict)} samples for hybrid splitter")
            else:
                print(f"Warning: Secondary dataset file not found at {secondary_fp}")
                dataset.secondary_dataset = []

    elif dataset_name.lower() == 'dolly-15k-ab':
        #fp = os.path.join(config.data.root, 'databricks-dolly-15k_AB_meta_train.jsonl')
        fp = os.path.join(config.data.root, 'databricks-dolly-15k_AB_meta_train_Adominance.jsonl')
        ...
        # my modification to quickly account for training on everything but summarization
        train_data_dict = [
            x for x in list_data_dict if x["category"] != "summarization"
        ]    

        dataset = LLMDataset(train_data_dict, tokenizer)
        #dataset = LLMDataset(list_data_dict, tokenizer)

    elif dataset_name.lower() == 'dolly-15k-b':
        fp = os.path.join(config.data.root, 'databricks-dolly-15k_Style_Childish.jsonl')
