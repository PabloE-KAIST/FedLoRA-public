...
class FSChatBot(object):
    ...
    def __init__(self, config):
        ...
        """
        self.config = config
        # Honor the chat template at eval time iff the model was trained with it
        # (cfg.llm.use_chat_template). Otherwise the instruct model is prompted
        # in a format it never saw during fine-tuning → degraded generation.
        self.use_chat_template = getattr(config.llm, 'use_chat_template', False)
        ...
        self.use_chat_template = self.use_chat_template and \
    ...
    def _build_prompt2(self, input_text, context=None):
        """
        Builds a prompt template for the input text.

        Args:
            input_text: A string representing the user's input text.
            context: An optional string representing the context/input for the task.

        Returns:
            A string representing the source text with a prompt template.
        """
        if getattr(self, 'use_chat_template', False):
            # Match the training-time formatting for instruct models: fold any
            # context into the user turn and open (empty) the assistant turn.
        ...
        if context and context.strip():
            source = {'instruction': input_text, 'input': context}
            return PROMPT_DICT['prompt_input'].format_map(source)
        else:
            source = {'instruction': input_text}
            return PROMPT_DICT['prompt_no_input'].format_map(source)
    ...
    def predict2(self, input_text, context=None, use_history=True, use_prompt=True):
        """
        Generates a response for the input text using the model.
        ...
        """
        if use_prompt:
            input_text = self._build_prompt(input_text, context)
        text_ids = self.tokenizer.encode(input_text, add_special_tokens=False)
        self.history.append(text_ids)
        input_ids = []
        if use_history:
    ...
    @torch.no_grad()
    def generate_old(self, input_text, generate_kwargs={}):
        """
    ...
    @torch.no_grad()
    def generate(self, input_text, context=None, generate_kwargs={}):
        """
        Generates a response for the input text using the model and
        additional arguments.
        ...
        Returns:
            ...
            returned.
        """
        # Build prompt: with chat template (instruct models) always wrap, even
        # when there is no context; otherwise only wrap when a context exists.
        if getattr(self, 'use_chat_template', False):
            input_text = self._build_prompt2(input_text, context)
        elif context and str(context).strip():
            input_text = self._build_prompt2(input_text, context)
