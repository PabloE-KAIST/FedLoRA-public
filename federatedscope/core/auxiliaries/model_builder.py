...
def get_model(config, local_data=None, backend='torch'):
    ...
    if model_config.type.lower() in \
            ...
            model_config.type.lower().endswith('_vlm'):
        input_shape = None
    ...
    # Only warn for models that actually *need* input_shape
    if input_shape is None and not (model_config.type.lower().endswith('_llm') or model_config.type.lower().endswith('_vlm')):
    ...
    elif '@vlm' in config.data.type.lower():
        from federatedscope.vlm.model import get_vlm_model
        model = get_vlm_model(config)
    elif '@glue' in config.data.type.lower():
        from federatedscope.glue.model import get_glue_model
        model = get_glue_model(config)
