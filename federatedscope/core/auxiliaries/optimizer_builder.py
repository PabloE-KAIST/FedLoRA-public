        ...
        #for QLoRA
            ...
            if isinstance(model, torch.nn.Module):
                ...
                return getattr(bitsandbytes.optim, type)(model, lr,
                                                         **tmp_kwargs)
        #if using adamw_8bit
        elif 'AdamW8bit' in type:
            import bitsandbytes
            # Remove unsupported parameters for AdamW8bit
            adamw8bit_kwargs = tmp_kwargs.copy()
            unsupported_params = ['momentum', 'nesterov']  # SGD-specific parameters
            for param in unsupported_params:
                if param in adamw8bit_kwargs:
                    logger.warning(f"Removing unsupported parameter '{param}' for AdamW8bit optimizer")
                    del adamw8bit_kwargs[param]
