...
def get_splitter(config):
    ...
    if config.data.splitter == 'lda':
        ...
        vis_kwargs = kwargs.copy()
        vis_kwargs.update({
            'save_plot': getattr(config.data, 'save_plot', False),
            'exp_name': getattr(config, 'expname', 'experiment'),
            'plot_dir': getattr(config.data, 'plot_dir', './0_Results')
        })
        splitter = LDASplitter(client_num, **vis_kwargs)
    elif config.data.splitter == 'hybrid_lda':
        from federatedscope.core.splitters.generic import HybridLDASplitter
        # Add visualization parameters
        vis_kwargs = kwargs.copy()
        vis_kwargs.update({
            'save_plot': getattr(config.data, 'save_plot', False),
        ...
        splitter = HybridLDASplitter(client_num, **vis_kwargs)
    # graph splitter
        ...
        splitter = MetaSplitter(client_num, **vis_kwargs)
    else:
