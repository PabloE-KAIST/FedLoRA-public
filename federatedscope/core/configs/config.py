...
class CN(CfgNode):
    ...
    def ready_for_run(self, check_cfg=True):
        ...
        Args:
            check_cfg: whether enable ``assert_cfg()``
        """
        # If already ready, skip processing
        if hasattr(self, 'is_ready_for_run') and self.is_ready_for_run:
            return
        
        # If already frozen, assume it's ready (freeze() always calls ready_for_run() first)
        if self.is_frozen():
            return
    ...
    def freeze(self, inform=False, save=True, check_cfg=True):
        """
        ...
        """
        # If already frozen, return early
        if self.is_frozen():
            return
        ...
        if save:  # save the final cfg
            ...
            if inform:
                #logger.info("Logging the full config is commented out.")
...
def init_global_cfg(cfg):
    ...
    # Global debug toggle for guarding debug-only code paths
    cfg.debug = False
