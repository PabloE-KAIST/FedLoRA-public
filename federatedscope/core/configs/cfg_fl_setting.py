...
def extend_fl_setting_cfg(cfg):
    ...
    # Federation-correctness guards for the hetero-LoRA download path (2026-07 fix).
    # assert_download_consumed: raise if any distributed-format LoRA download key cannot
    #   be matched to a trainable model param (guards the silent-drop bug where HetLoRA /
    #   AdaSparse never applied the aggregated global adapter). Default True.
    # assert_download_tensor_equality: additionally verify at round start that the model's
    #   LoRA equals the applied download (per-round cost; default False, on for tests).
    cfg.federate.assert_download_consumed = True
    cfg.federate.assert_download_tensor_equality = False
    ...
    # ---------------------------------------------------------------------- #
    # Communication bandwidth simulation options
    # Shared configuration for all methods (FAH-QLoRA, AdaSparse-LoRAv2, etc.)
    # ---------------------------------------------------------------------- #
    cfg.federate.communication = CN()
    cfg.federate.communication.enabled = True  # Master switch for bandwidth simulation
    
    # Mode options:
    #   - 'static': Sample once at initialization, values remain fixed for all rounds
    #   - 'dynamic': Re-sample (or advance trace) once per round
    cfg.federate.communication.mode = 'dynamic'
    
    # Source options:
    #   - 'generated': Random sampling from [uplink_min_mbps, uplink_max_mbps], fixed downlink
    #   - 'trace': Read from network trace CSV files
    cfg.federate.communication.source = 'generated'
    
    # Network trace configuration (used when source='trace')
    cfg.federate.communication.network_trace_path = ''  # Path to network trace directory
    cfg.federate.communication.network_trace_distribution = CN(new_allowed=True)  # {class_name: percentage}

    # Optional dedicated communication seed. When None, falls back to cfg.seed.
    # This isolates bandwidth sampling from method-specific randomness.
    cfg.federate.communication.seed = 43
    
    # Generated bandwidth configuration (used when source='generated')
    # Values are in Mbps (converted to kbit/s internally)
    cfg.federate.communication.uplink_min_mbps = 5
    cfg.federate.communication.uplink_max_mbps = 20
    cfg.federate.communication.downlink_mbps = 50
    
    # Symmetric UL/DL option: when true, use downlink value for uplink as well
    cfg.federate.communication.force_symmetric_ul_dl = False
    
    # Export history to CSV file
    cfg.federate.communication.export_history = True

    # TC JSON bandwidth profiles (used when source='tc_json')
    cfg.federate.communication.tc_json_ul_path = ''
    cfg.federate.communication.tc_json_dl_path = ''
    ...
    # ---------------------------------------------------------------------- #
    cfg.monitor = CN()
    # system_metrics_mode: "legacy" or "extended"
    # - "legacy": original flat format (fl_end_time_minutes, total_model_size, etc.)
    # - "extended": generic per-client extended format with per-round communication,
    #   timing, memory breakdown, and server-side aggregate totals (id 0 entry)
    cfg.monitor.system_metrics_mode = "extended"
...
def assert_fl_setting_cfg(cfg):
    ...
    # Validate monitor.system_metrics_mode
    if hasattr(cfg, 'monitor') and hasattr(cfg.monitor, 'system_metrics_mode'):
        if cfg.monitor.system_metrics_mode not in ['legacy', 'extended']:
            raise ValueError(
                f'monitor.system_metrics_mode must be "legacy" or "extended", '
                f'but got "{cfg.monitor.system_metrics_mode}".'
            )
