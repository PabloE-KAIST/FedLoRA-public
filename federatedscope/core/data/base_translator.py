    ...
    def split_to_client(self, train, val, test):
        ...
        if len(train) > 0:
            ...
            if self.global_cfg.data.consistent_label_distribution:
                #Modification to handle dictionary format in data, where category can be accessed as j['categories']
                try:
                    #train_label_distribution = [[j[1] for j in x] for x in split_train]
                    if isinstance(split_train[0][0], dict):
                        train_label_distribution = [[j['categories'] for j in x] for x in split_train]
                    else:
                        train_label_distribution = [[j[1] for j in x] for x in split_train]
                except:
                    logger.warning(
