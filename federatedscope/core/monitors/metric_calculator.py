...
def _safe_tensor_to_numpy(value):
    """Convert torch tensors to NumPy safely for metric computation.

    Floating tensors are promoted to float32 before conversion so that
    torch.bfloat16 and torch.float16 values can be consumed by NumPy/scipy.
    """
...
class MetricCalculator(object):
    ...
    def _check_and_parse(self, ctx):
        ...
        y_true = _safe_tensor_to_numpy(ctx.ys_true)

        y_prob = None
        if ctx.get('ys_pred', None) is not None:
            y_pred = _safe_tensor_to_numpy(ctx.ys_pred)
            ...
            y_prob = _safe_tensor_to_numpy(ctx.ys_prob)
...
def eval_mcc(y_true, y_pred, **kwargs):
...
def eval_pearson(y_true, y_prob, **kwargs):
    y_true = np.asarray(y_true).reshape(-1)
    y_prob = np.asarray(y_prob).reshape(-1)
    try:
        return pearsonr(y_true, y_prob)[0]
    except Exception as e:
        logger.warning(f'Unable to compute pearson: {e}')
        return 0.0


def eval_spearman(y_true, y_prob, **kwargs):
    y_true = np.asarray(y_true).reshape(-1)
    y_prob = np.asarray(y_prob).reshape(-1)
    try:
        return spearmanr(y_true, y_prob)[0]
    except Exception as e:
        logger.warning(f'Unable to compute spearman: {e}')
        return 0.0
...
SUPPORT_METRICS = {
    ...
    'std': (None, False),
    'mcc': (eval_mcc, True),
    'pearson': (eval_pearson, True),
    'spearman': (eval_spearman, True),
    'rouge': (None, True),
